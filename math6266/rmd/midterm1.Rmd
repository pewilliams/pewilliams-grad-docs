...
$$||Y - X^{\intercal}\theta||^2 = G(\theta) = (Y - X^{\intercal}\theta)^{\intercal}(Y - X^{\intercal}\theta) = YY^{\intercal} - 2\theta^{\intercal}XY + \theta^{\intercal}XX^{\intercal}Y$$ with gradient, $$\nabla G(\theta)  = -2XY + 2\theta^{\intercal}XX^{\intercal}Y$$ Setting this expression equal to zero leads to estimator $\hat{\theta}=\hat{\theta}_{OLS} = (XX^{\intercal})^{-1}XY$, which leads to response estimator $\hat{Y} = X^{\intercal}\hat{\theta} = X^{\intercal}(XX^{\intercal})^{-1}XY$.
