...
and
$$Var(\hat{Y} - Y) = Var(\hat{Y}) + Var(Y) = Var(\Pi Y) + \sigma^2 I_n = Var(\Pi (X^{\intercal}\theta^* + \varepsilon)) + \sigma^2I_n = Var(\Pi\varepsilon) + \sigma^2I_n = \sigma^2(\Pi + I_n)$$
and assume that $\hat{Y} - Y \sim N(0, \sigma^2(\Pi + I_n))$, and $\frac{(\Pi + I_n)^{-1/2}(\hat{Y} - Y)}{\sigma} \sim N(0, I_n)$ Using this information we can set up a confidence region for $\hat{Y}$, assuming the statistic,
$$\frac{\frac{(\hat{Y} - Y)^{\intercal}(\Pi + I+N)^{-1}(\hat{Y} - Y)}{\sigma^2}\frac{1}{n}}{\frac{||Y - X^{\intercal}\hat{\theta}||}{\sigma^2}\frac{1}{n-p}} =  \frac{(n-p)(\hat{Y} - Y)^{\intercal}(\Pi + I_n)^{-1}(\hat{Y} - Y)}{n||Y - X^{\intercal}\hat{\theta}||}\sim F_{n,n-p}$$
With this statistic we can set up a $\%(100-\alpha)$ confidence region, using the $\alpha^{th}$ quantile of the F-distribution, $Q_{F_{(p,n-p)}}^*$, such that 
$$P( \frac{(n-p)(\hat{Y} - Y)^{\intercal}(\Pi + I_n)^{-1}(\hat{Y} - Y)}{n||Y - X^{\intercal}\hat{\theta}||} < Q_{F_{(p,n-p)}}^* ) = 1-\alpha$$
