...
An example of a strict inequality: the functions $f, g$, on the set $X = \{x: 0<x<1\}$ for $f(x) = g(x) = x$. Clearly $\text{inf}\{x: 0<x<1\} = 0$, thus $\text{inf}\{f(x): x \in X\} + \text{inf}\{g(x): x \in X\} = 0$ which is less than $\text{inf}\{f(x) + g(x): 0<x<1 \} > 0$, since, $f(x) > 0$ and $g(x) > 0\ \ \forall x \in X$. 
