...
If we take $G = (0,2)$, and open set, $h^{-1}(G) = \{x \in \mathcal{D}(f): h(x) \in G\} = [0,1]$, a closed set. If we take $F = [-2,-1]$, a closed set, the inverse image, $h^{-1}(F) = \{x \in \mathcal{D}(f): h(x) \in F\} = [0,1]$  is the union of two open sets $(-\infty,0) \cup (1,+\infty)$ which is open. 
