...
*G. A function $g:\mathbb{R} \to \mathbb{R}^p$ is periodic if there exists a snumber $p>0$ such that $g(x + p) = g(x)$ for all $x \in \mathbb{R}$. Show that a continuous periodic function is bounded and uniformly continuous on $\mathbb{R}$.*   
