...
*F. Show that $f(x) = \frac{1}{x^2},\ \mathcal{D}(f) = \{x \in \mathbb{R}: x > 0\}$ is on uniformly continuous on its domain.*
*G. A function $g:\mathbb{R} \to \mathbb{R}^p$ is periodic if there exists a snumber $p>0$ such that $g(x + p) = g(x)$ for all $x \in \mathbb{R}$. Show that a continuous perioddic function is bounded and uniformly continuous on $\mathbb{R}$.*  
