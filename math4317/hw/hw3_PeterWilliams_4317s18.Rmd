...
Since $x > 0,\ \frac{1}{nx}$ is defined for all $n \in \mathbb{N} \implies \lim{(f_n(x))}$ exists.   
...
We assume $\lim(g_n(x)) = g(x) = 0$. For the case $0 \leq x \leq \frac{1}{n},\ |g_n(x) - g(x)| = |nx - 0| = nx \implies 0 \leq nx \leq \frac{n}{n} = 1$. For $n \geq K(\varepsilon,x) \in \mathbb{N}$ we have $nx \leq K(\varepsilon,x) x < \varepsilon,\ \varepsilon > 0$. 
