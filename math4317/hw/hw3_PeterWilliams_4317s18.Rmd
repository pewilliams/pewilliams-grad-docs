...
As an example, if we take sequences $X = (x_n) = (-1)^n = (-1,1,-1,...)$ and $Y = (y_n) = (-1)^{n+1} = (1,-1,1,...)$ then their product $X \cdot Y = (-1,-1,-1,...)$ which converges to $-1 \implies$ that if the product $X \cdot Y$ converges, but each sequence $X$ and $Y$ does not have a limit, divergea. 

As another example, in the case of the constant sequences $X = (x_N) = (1,1,...)$, and $Y = (y_n) = (2,2,...)$, $X \cdot Y$ is the constant sequence $(2,2,...)$ which converges to $2$ which equals $\lim X \cdot \lim Y$. Therefore the convergence of $X \cdot Y$ converges does not necessarily mean that each sequence converges.  
...
*J. Show directly that the the following are not Cauchy sequences.*  
(a)  $((-1)^n$  
(b)  $(n + \frac{(-1)^n}{n})$  
(c)  $(n^2)$
